...
- **New tool: `steam_analyze_app_reviews`.** Reads up to 20,000 of a game's newest reviews (or the last N days) and reports what a score alone can't: sentiment over time, by language and by playtime at review, and for Steam purchases vs key activations vs free copies, early-access reviews, reviewers who played mostly on Steam Deck, reviewers who refunded it, and reviews the developer answered. It also returns review length and a few of the newest and most helpful reviews on each side. Reviews are de-duplicated across pages, a failed page keeps what was already counted, and a large corpus can be read in installments by passing `next_cursor` back as `cursor`. No key needed; 2,000 reviews take about 5 seconds.
